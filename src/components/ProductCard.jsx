...
export const ProductCard = ({ product }) => {
  ...
  // Realistic High-Depth CSS/SVG Product Renderers per finish (Part 2 Fix)
  const renderRealisticCase = () => {
    switch (product.id) {
      case 'case-solar':
        return (
          <div className="w-28 h-48 rounded-[22px] border border-gold/40 bg-stone/60 shadow-[0_12px_24px_rgba(0,0,0,0.8)] relative flex flex-col justify-between p-3 group-hover:scale-105 transition-transform duration-300 backdrop-blur-sm">
            {/* Camera Module with Anodized Gold Ring */}
            ...
            {/* Subtle Debossed Tone-on-Tone Logo on Bottom Corner */}
            <div className="self-start text-[8px] font-mono tracking-widest text-gold/30 font-bold uppercase select-none">
              DUAT
            </div>
          </div>
        );
      ...
      case 'case-ember':
        return (
          <div className="w-28 h-48 rounded-[22px] border border-[#4A2418] bg-gradient-to-b from-[#2A1610] to-[#1A0C0A] shadow-[0_12px_24px_rgba(0,0,0,0.8)] relative flex flex-col justify-between p-3 group-hover:scale-105 transition-transform duration-300">
            ...
            {/* Debossed Tone-on-Tone Logo */}
            <div className="self-start text-[8px] font-mono tracking-widest text-[#4A2418] font-bold uppercase select-none">
              DUAT
            </div>
          </div>
        );
      ...
      case 'case-eclipse':
        return (
          <div className="w-28 h-48 rounded-[22px] border border-grave bg-stone shadow-[0_12px_24px_rgba(0,0,0,0.8)] relative flex flex-col justify-between p-3 group-hover:scale-105 transition-transform duration-300">
            ...
            {/* Debossed Tone-on-Tone Logo */}
            <div className="self-start text-[8px] font-mono tracking-widest text-ash/40 font-bold uppercase select-none z-10">
              DUAT
            </div>
          </div>
        );
      ...
      default:
        return (
          <div className="w-28 h-48 rounded-[22px] border border-coal bg-[#0A0A0A] shadow-[0_12px_24px_rgba(0,0,0,0.9)] relative flex flex-col justify-between p-3 group-hover:scale-105 transition-transform duration-300">
            {/* Camera Module (Monochrome Matte Black) */}
            ...
            {/* Debossed Tone-on-Tone Logo */}
            <div className="self-start text-[8px] font-mono tracking-widest text-coal font-bold uppercase select-none">
              DUAT
            </div>
          </div>
        );
  ...
  // Other Category Graphic Renderers (Non-Case)
  const renderCategoryGraphic = () => {
    if (product.category === 'cases') return renderRealisticCase();
  ...
  return (
    <div className="bg-stone border border-grave flex flex-col justify-between group relative overflow-hidden transition-all duration-300 hover:border-gold/60 cursor-pointer">
